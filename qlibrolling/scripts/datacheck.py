...
class DataHealthChecker:
    ...
    def __init__(
        ...
        self.is_qlib_bin = True if  qlib_dir else False # 是否检查bin数据
        # Set up logging
        self.log = "G:\\qlibtutor\\scripts\\datacheck.log"
        print('qtb======================')
        print('qtb======================')
        logging.basicConfig(filename=self.log,filemode='w',level=logging.WARNING)
    ...
    def load_qlib_data(self):
        ...
        required_fields = ["$open", "$close", "$low", "$high", "$volume", "$vwap","$apt","$factor"]

        for instrument in instrument_list:
            
            df = D.features([instrument], required_fields,start_time=self.start_date, freq="day")
    ...
    def check_required_columns(
        ...
        #         f"{filename}: Missing columns {missing_required_columns} of required columns {required_columns} df columns{df.columns}."
        ...
        if not self.is_qlib_bin:
            ...
            if  "$factor" not in df.columns: # 
                ...
                logging.warning(
                    ...
                    f"{filename}: Missing '$factor' column, trading unit will be disabled df.columns {df.columns}."
...
if __name__ == "__main__":
    ...
    dc = DataHealthChecker(qlib_dir=r'G:\qlibtutor\qlib_data\testdata', start_date="2008-01-01") # qlib_dir=r'G:\qlibtutor\qlib_data\rq_cn_data' ,csv_path = r'G:\qlibtutor\qlib_data\rq_cn_data\csv'
    dc.check_data()