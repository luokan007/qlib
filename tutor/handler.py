...
class MyAlpha(DataHandlerLP):
    ...
    def get_label_config(self):
        # return (["Ref($open, -2)/Ref($close, -1) - 1"], ["LABEL0"])
    ...
    def get_feature_config(self):
        ...
        fields = ["Ref($close, 10)/$close", "Ref($high, 10)/$close", "Ref($volume, 10)/($volume+1e-12)"]
        names = ["CLOSE10", "HIGH10","VOLUME10"]


        return fields, names

class MyAlphaExp(DataHandlerLP):
    def __init__(
        self,
        instruments="csi500",
        start_time=None,
    ...
    def get_feature_config(self):
        ...
        # fields = ["Ref($close, 10)/$close", "Ref($high, 10)/$close", "Ref($volume, 10)/($volume+1e-12)"]
        fields = [Ref(Feature('close'), 10)/Feature('close'), Ref(Feature('high'), 10)/Feature('close'), Ref(Feature('volume'), 10)/(Feature('volume')+1e-12)]
        names = ["CLOSE10", "HIGH10","VOLUME10"]
...
class MyAlpha158Ext(Alpha158):
    
    def get_feature_config(self):
        fields, names = [],[]
        fields, names  = super().get_feature_config()
        fields += ['Ref($turnover, 1)/$turnover', '$turnover/$turnover']
        names += ['TURNOVER1', 'TURNOVER0']
        return fields, names
