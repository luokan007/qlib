# title: GoldSparrow_1.1.py
# updated: 2024.10.22
# change log:
#   - 设置了全局config变量
#   - 训练、预测分离，以函数形式封装，通过全局config配置所需环节
#   - 模型分析以函数形式封装，用quantstate输出HTML文件
#   - 支持LightGBM模型，未来（1.2）支持LSTM、transformer、XGB等模型
...
def pred_backtest_process(dataset,model) -> None:
    ...
    with R.start(experiment_name="LightGBM_Alpha180"):
        # 当前实验的实验记录器：预测实验记录器
