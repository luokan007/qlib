...
class DDGDA_rolling_benchmark(DDGDA):
    def __init__(self,
...
def Offline_Evaluate(is_process_data = True) -> None:
    ...
    exp_name = "ddg_bench_new"  # 合并预测结果pred.pkl存放mlflow实验名
    rb = DDGDA_rolling_benchmark(
    ...
    # prepare the meta model for rolling ---------
        ...
        # super().run()   ----------->> call the rolling.run()
    ...
    rb._dump_data_for_proxy_model()
    print("dump meta data for proxy model finished:",datetime.now())
    rb._dump_meta_ipt()
    print("dump meta data for meta model finished:",datetime.now())
    rb._train_meta_model()
    print("train meta model finished:",datetime.now())
    rb._train_rolling_tasks()
    print("train rolling tasks finished:",datetime.now())
    rb._ens_rolling()
    print("ens rolling finished:",datetime.now())
    rb._update_rolling_rec()
    print("update rolling record finished:",datetime.now())
